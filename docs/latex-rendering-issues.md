...
> 生成日期：2026-05-12 | 分析样本：ROBOT 课件 5 章笔记 | 最后更新：2026-05-13（Python 验证修正）

## 问题总览

| 类别 | 严重度 | 影响范围 | 检测状态 |
|------|--------|---------|---------|
| `$$` 后空行 | CRITICAL | Ch2-5，54/58 个公式块 (93%) | 检测规则缺失 / 修复逻辑有 Bug |
| 代码块包裹 | HIGH | Ch2 | 已检测，截断时修复失败 |
| `$$` 前空格 | MEDIUM | Ch2-4，散在 | 未检测/未修复 |
...
### 短期（prompt 修复 + post_processor 增强）

| # | 修复项 | 位置 | 难度 | 优先级 |
|---|--------|------|------|--------|
| P1 | prompt: 添加 LaTeX 格式约束 + 移除 ````markdown` 模板包裹 | `config/prompts.py:50, 81` | 低 | **最高** |
| 1 | `fix_latex_format`：新增 `$$` 后空行移除（`r'\$\$\n\n+' → '$$\n'`）| `post_processor.py:298` | 低 | 高 |
| 2 | `fix_latex_format`：**删除 Fix 4**（主动破坏正确格式，无合理用例）| `post_processor.py:316-320` | 低 | 高 |
| 3 | `remove_code_block_wrapper`：无闭合标签时仍移除开标签 | `post_processor.py:84` | 低 | 高 |
| 4 | `fix_latex_format`：Fix 3 正则增强（处理 tab 和多空格 `r'[ \t]+(\$\$\n)'`）| `post_processor.py:308` | 低 | 中 |
| 5 | `check_latex_format`：新增 `$$` 后空行检测（`r'\$\$\n\n'`）| `post_processor.py:336` | 低 | 中 |
| 6 | `check_latex_format`：新增 `$$` 前空格检测（`r' \$\$'`）| `post_processor.py:336` | 低 | 中 |
| 7 | 测试：新增覆盖 `$$` 后空行、前空格、Fix 1+4 冲突的测试用例 | `tests/test_post_processor.py:56` | 低 | 中 |

### 中期（prompt 层面）

| # | 方案 | 说明 |
|---|------|------|
| 8 | few-shot 示例 | 提供正确/错误 LaTeX 格式对比示例 |

### 长期（结构化输出）

| # | 方案 | 说明 |
|---|------|------|
| 9 | 结构化章节输出 | 将公式内容与文本内容分离，模板化渲染 |
| 10 | 引入 LaTeX 校验器 | 生成后自动编译验证公式语法 |
