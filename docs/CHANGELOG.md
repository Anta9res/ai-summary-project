...
## v1.4.0 — 章节检测三路互补 (2026-05-14)

### 功能 — 节复位推断（策略3）

- 新增 `_infer_chapters_from_section_reset()`：检测节编号 N→1 复位，推断无显式"第X章"标题的章边界
- 近邻排除：已知章 300 字符内的复位不产生推断
- 推断章标题自动从第一节标题推导（含"概述/概说/概念"后缀剥离）
- 新增中文数字转换工具：`_chinese_to_int()` / `_int_to_chinese()`
- 新增 `MARKDOWN_SECTION_PATTERN` 正则提取节标题
- `_dedup_and_sort()` 修正：保留预置位置（position>0），不覆盖推断章边界
- 民法验证：6/12 → 12/12 章完整检测，12 笔记生成，63KB 完整复习笔记

### 测试

- `test_chapter_splitter.py` 新增 13 个测试用例（中文数字转换 6 + 节复位推断 7）
- 全量 71 测试通过
